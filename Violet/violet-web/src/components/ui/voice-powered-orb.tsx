...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
    ...
    // Refs for animation loop to access latest props without re-init
  ...
  `;
    ...
    useEffect(() => {
        if (mode === "listening") {
            ...
            stopMicrophone(); // Stop mic if not listening
        }
    }, [mode]);
    ...
    useEffect(() => {
        ...
        try {
            ...
            const update = (t: number) => {
                ...
                program.uniforms.hue.value = hueRef.current; // Use ref

                const currentMode = modeRef.current;

                let targetRotSpeed = baseRotationSpeed;
                let targetHover = 0;
                ...
                if (currentMode === "listening") {
                    if (analyserRef.current) {
                        ...
                        targetRotSpeed = baseRotationSpeed + (voiceLevel * maxRotationSpeed);
                        targetHover = Math.min(voiceLevel * 2.0, 1.0);
                        targetIntensity = Math.min(voiceLevel * maxHoverIntensity, maxHoverIntensity);
                    }
                } else if (currentMode === "speaking") {
                    const pulse = (Math.sin(time * 8.0) * 0.5 + 0.5);
                    targetRotSpeed = baseRotationSpeed * 1.5;
                    targetHover = pulse * 0.8;
                    targetIntensity = pulse * 0.6;
                    ...
                    targetRotSpeed = baseRotationSpeed;
                    targetHover = breath * 0.1;
                    targetIntensity = breath * 0.05;
                }

                currentRot += dt * targetRotSpeed;
                program.uniforms.hover.value = targetHover;
                program.uniforms.hoverIntensity.value = targetIntensity;
            ...
            return () => {
                ...
                // Simple canvas removal
                if (container && glContext && glContext.canvas && container.contains(glContext.canvas)) {
    ...
    }, [maxRotationSpeed, maxHoverIntensity]); // Reduced deps to avoid re-init
